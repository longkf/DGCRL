...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./save/model.pt')
    ...
    for run in range(args.runs):
        ...
        torch.save(test_data.cpu(), './save/test_data.pt')
        print(test_data)
        ...
        pred = F.cosine_similarity(src, dst)
        torch.save(pred.cpu(), './save/pred.pt')
        ...
        for epoch in range(1, 1 + args.link_epochs):
            ...
            if roc >= best_roc:
                best_roc = roc
                torch.save(decoder, args.save_path)
                print('*', end='')
            print(f'epoch: {epoch} loss: {loss} roc: {roc} ap: {ap}')
        decoder = torch.load(args.save_path)
        roc, ap = link_test()
